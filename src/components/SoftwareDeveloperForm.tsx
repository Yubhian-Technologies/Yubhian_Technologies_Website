...
const SoftwareDeveloperForm: React.FC = () => {
  const navigate = useNavigate();
  const [formData, setFormData] = useState<SDEFormData>({
    fullName: "",
    email: "",
    phone: "",
    location: "",
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const GOOGLE_APPS_SCRIPT_URL =
      "https://script.google.com/macros/s/AKfycbyr-3lS_g5z46pWNqrIGp3qghTBb-2hxMRvCcX9WRchyv3sP_Ocyf0aTzLXDbbdg7dW/exec";
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="flex space-x-4 pt-6">
          ...
          <button
            ...
            className="flex-1 py-3 px-6 bg-gradient-to-r from-violet-400 to-purple-400 text-white font-semibold rounded-lg disabled:opacity-50"
          >
            Submit Application
          </button>
        </div>
    ...
    <div className="bg-black min-h-screen py-12 lg:py-20 flex items-center justify-center">
      <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl">
        <div className="p-6 border-b border-gray-700">
          <h3 className="text-2xl font-bold text-white">Apply for Software Developer</h3>
          <p className="text-gray-400 mt-1">Full-time Position</p>
        </div>
        {renderContent()}
      </div>
    </div>
...
export default SoftwareDeveloperForm;
